...
pgxmldbgui::pgxmldbgui()
    : QMainWindow( 0, "pgxmldbgui", WDestructiveClose )
{
...
void pgxmldbgui::newDoc()
{
    pgxmldbgui *ed = new pgxmldbgui;
    ed->setCaption(tr("Qt Example - Application"));
    ed->show();
}

void pgxmldbgui::choose()
{
...
void pgxmldbgui::load( const QString &fileName )
{
    QFile f( fileName );
...
void pgxmldbgui::closeEvent( QCloseEvent* ce )
{
    if ( !e->isModified() ) {
	ce->accept();
	return;
